...
export class ClientComponent implements AfterViewInit, OnInit {
  public readonly displayedColumns = ['id', 'cpf', 'name'];
  ...
  public edit(row: Client): void {
    this.clientService.getOne(row.id).subscribe((data: any) => {
      if (data.success) {
        const dialogRef = this.dialog.open(FormsComponent, {
          width: '400px',
          data: { title: 'Update client', action: 'edit', data: data.data }
        });
  ...
  public save(): void {
    // const dialogRef = this.dialog.open(FormsComponent, {
    //   width: '100vh',
    //   data: { title: 'Adicionar um cliente', action: 'save' }
    // });
    // dialogRef.afterClosed().subscribe(result => {
    //   if (result) {
    //     this.paginator._changePageSize(this.paginator.pageSize);
